...
def trainAndReport(comm, nprocs, rank, Xtrain, ytrain, Xtest, ytest, params):
    ...
    start_time = time.time() #trainning time starter add by bidong
    lossTrack = list()
    nn.fit(comm, nprocs, rank, Xtrain, ytrain, lr, M, seed, threshold, T, lossTrack)
    elapsed_time = time.time() - start_time  #trainning time calculated by bidong
    return (np.array(lossTrack), nn.calculateLoss(comm, nprocs, rank, Xtest, ytest), elapsed_time)
...
def main():
    ...
    if rank == 0:
        print("Final Test MSE:", testLoss)
        print(f"Total training time (s): {trainTime:.2f}")
